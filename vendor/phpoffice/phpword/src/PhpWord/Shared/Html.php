...
class Html
{
    ...
    private static function parseTable($node, $element, &$styles, $argument1)
    {
        $styles['paragraph'] = self::parseInlineStyle($node, $styles['paragraph']);
        $styles['font'] = self::parseInlineStyle($node, $styles['font']);
        $styles['table'] = self::parseInlineStyle($node, $styles['table']);
        if ('addCell' == $argument1) {
            // Always with border
            $styles['table'] = array_merge($styles['table'], array(
                'borderTopSize' => '1',
                'borderRightSize' => '1',
                'borderBottomSize' => '1',
                'borderLeftSize' => '1',
            ));
            $styles['paragraph'] = array_merge($styles['paragraph'], array(
                'alignment' => 'center',
            ));
            if ('th' == $node->nodeName) {
                $styles['font'] = array_merge($styles['font'], array(
                    'bold' => true,
                ));
            }
        }
        if (strpos($argument1, 'skip') === 0) {
            $newElement = $element;
        } else {
            switch ($argument1) {
                case 'addTable':
                    $newElement = $element->addTable($styles['table']);
                    break;
                case 'addRow':
                    /** @var \PhpOffice\PhpWord\Element\Table $element */
                    $newElement = $element->addRow(null, $styles['table']);
                    break;
                case 'addCell':
                    /** @var \PhpOffice\PhpWord\Element\Row $element */
                    $newElement = $element->addCell(null, $styles['table']);
                    break;
                default:
                    $newElement = null;
            }
